...
function omn_superadmin_overview_page_default() {
	...
	$query_messages = 'SELECT * FROM '.$wpdb->base_prefix.'omn_messages';
...
function omn_create_message( $title, $text, $link, $author_id ) {
	...
	$inok = $wpdb->insert( $wpdb->base_prefix.'omn_messages', 
		array( 'title' => $title, 'text' => $text, 'link' => $link, 'author' => $author_id, 'date' => time() ),
		array( '%s', '%s', '%s', '%d', '%d' ) 
	);
			...
			<?php
				foreach( $unread_messages as $message ) {
					...
					<tr>
						...
						<td><?php echo $authordata->user_login; ?></td>
						<td><?php echo '<a href="index.php?page=omn-messages&action=read&id='.$message->id.'">přečteno</a>'; ?></td>
					</tr>
					<?php
				}
			?>
